...
try {
    // Estatísticas completas para gestor
    $stats = [
        'cancelado' => 0,
        'orcamento' => 0,
        'arte' => 0,
    ...
    $statusQueries = [
        'cancelado' => "SELECT COUNT(*) FROM pedidos WHERE status = 'cancelado'",
        'orcamento' => "SELECT COUNT(*) FROM pedidos WHERE status = 'orcamento'",
        'arte' => "SELECT COUNT(*) FROM pedidos WHERE status = 'arte'",
        ...
        'urgentes' => "SELECT COUNT(*) FROM pedidos WHERE urgente = true AND status NOT IN ('entregue', 'cancelado')"
    ];
...
function abrirNovoPedidoModal(clienteId = null) {
    const url = clienteId ? 
        `pedido_novo.php?cliente_id=${clienteId}` : 
        'pedido_novo.php';
                        ...
                        <tr>
                            <td colspan="10" class="text-center py-12">
                                <div class="flex flex-col items-center gap-4">
                                    ...
                                    <a href="pedido_novo.php" class="bg-blue-600 hover:bg-blue-700 text-white font-bold px-4 md:px-6 py-2 md:py-3 rounded-lg inline-flex items-center gap-2">
